 ...
 *
 * Todo el texto va en blanco sobre la noche, con el acento en mango, igual que
 * la card de descarga del sitio.
 */
    ...
    <>
      <div className={`${s.cabecera} absolute inset-x-0 top-0 flex items-start justify-between px-[22px] pt-6 desk:px-[clamp(28px,4vw,64px)] desk:pt-8`}>
        <Image
          ...
          priority
          unoptimized
          className={`${e.entra} block h-auto w-[132px] desk:w-[164px]`}
        />
      </div>
      ...
      <div className={`${s.columna} absolute inset-x-0 top-0 flex flex-col items-center px-[22px] pt-[max(118px,15dvh)] text-center desk:pt-[clamp(84px,10vh,116px)]`}>
        <h1
          className={`${e.entra} ${s.titular} m-0 max-w-[16ch] font-display text-[clamp(34px,8.8vw,62px)] font-extrabold leading-[1.04] tracking-[-.03em] text-white [text-wrap:balance]`}
          style={{ animationDelay: "260ms", fontVariationSettings: '"opsz" 96' }}
        >
          Desde arriba, un punto. <em className="crd-accent-on-ink">De cerca, todo.</em>
        </h1>

        <p
          className={`${e.entra} m-0 mt-3.5 max-w-[44ch] text-[clamp(15px,1.5vw,18px)] font-medium leading-[1.5] text-white/78 desk:mt-4`}
        ...
        <div
          className={`${e.entra} ${s.acciones} mt-6 flex flex-col items-center gap-3 desk:mt-7 desk:flex-row desk:gap-3.5`}
