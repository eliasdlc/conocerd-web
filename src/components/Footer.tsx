...
export default function Footer() {
  return (
    // .crd-footer (globals.css): fuera del flujo mientras el recorrido móvil
    // está bloqueado, para que no quede nada bajo la pantalla que un gesto
    // pueda sacar a la vista.
    <footer className="crd-footer bg-[linear-gradient(135deg,var(--color-ink),var(--color-ink-2))] text-white">
      ...
      <div className="mx-auto flex max-w-[1100px] flex-wrap items-center justify-between gap-3 border-t border-white/10 px-[clamp(20px,5vw,56px)] py-4">
        ...
        <div className="flex gap-[18px]">
          ...
          <Link href="/eliminar-cuenta" className="inline-flex h-11 items-center text-xs text-white/70 no-underline">Eliminar cuenta</Link>
        </div>
      </div>
    </footer>
  );
